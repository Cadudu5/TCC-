...
def run_training_experiments():
    """
    Executa experimentos com XGBoost comparando grupos de features.
    Salva o melhor modelo e metadados, além de ranking de atributos.
    """
    ...
    # Cálculo de scale_pos_weight para desbalanceamento
    ...
    # --- Tabela consolidada ---
    print("\n" + "#" * 60)
    print("        TABELA DE MÉTRICAS POR EXPERIMENTO (XGBoost)")
    print("#" * 60)
    ...
    # --- Salvamento do melhor modelo, metadados e ranking de atributos ---
    if melhor_modelo is not None:
        print("\nSalvando o melhor modelo XGBoost, metadados e importâncias de atributos...")
        base_dir = os.path.dirname(os.path.abspath(__file__))
        artifacts_dir = os.path.join(base_dir, 'artifacts')
        os.makedirs(artifacts_dir, exist_ok=True)

        model_path = os.path.join(artifacts_dir, 'xgb_best.joblib')
        meta_path = os.path.join(artifacts_dir, 'xgb_best_meta.json')
        fi_path = os.path.join(artifacts_dir, 'xgb_best_feature_importances.csv')
        ...
        meta = {
            ...
            "xgb_params": dict(XGB_PARAMS) | {"scale_pos_weight": scale_pos_weight},
            "slic": slic_params,
        }
        with open(meta_path, 'w', encoding='utf-8') as f:
            json.dump(meta, f, ensure_ascii=False, indent=2)

        # Ranking de atributos do melhor modelo
        booster = melhor_modelo.get_booster()
        # Usa nomes de features preservados pelo pandas (colunas)
        gain_scores = booster.get_score(importance_type='gain')  # dict: feature_name -> score
        # Completa com zero para features sem ganho registrado
        feature_to_gain = {name: float(gain_scores.get(name, 0.0)) for name in melhor_features}
