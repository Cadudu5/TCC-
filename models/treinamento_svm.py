...
# Parâmetros do Modelo SVM
SVM_PARAMS = {
    'kernel': 'rbf',
    'C': 1.0,
    'gamma': 'scale',
    'class_weight': 'balanced',  # seguro mesmo com dataset balanceado
    'probability': False,  # defina True se precisar de probabilidades (mais lento)
}
...
def run_training_experiments():
    """
    Executa experimentos com SVM (StandardScaler + SVC) comparando grupos de features.
    ...
    # --- Tabela consolidada ---
    print("\n" + "#" * 60)
    print("        TABELA DE MÉTRICAS POR EXPERIMENTO (SVM)")
    print("#" * 60)
    ...
    if melhor_modelo is not None:
        print("\nSalvando o melhor modelo SVM e metadados...")
        base_dir = os.path.dirname(os.path.abspath(__file__))
        artifacts_dir = os.path.join(base_dir, 'artifacts')
        os.makedirs(artifacts_dir, exist_ok=True)

        model_path = os.path.join(artifacts_dir, 'svm_best.joblib')
        meta_path = os.path.join(artifacts_dir, 'svm_best_meta.json')
