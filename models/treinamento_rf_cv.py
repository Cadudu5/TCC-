...
def treinar_cross_validation(experimentos: dict, X: pd.DataFrame, y: pd.Series):
    ...
    for nome_exp, features in experimentos.items():
        ...
        for train_idx, test_idx in cv.split(X_subset, y_array):
            ...
            if nome_exp == next(iter(experimentos)):
                # Apenas no primeiro experimento registramos os tamanhos; são iguais para todos
...
def salvar_artifacts(modelo, features, melhor_nome_exp, melhor_f1, tabela_metricas):
    ...
    model_path = os.path.join(artifacts_dir, 'rf_best_cv.joblib')
    meta_path = os.path.join(artifacts_dir, 'rf_best_cv_meta.json')
    metrics_path = os.path.join(artifacts_dir, 'rf_cv_metricas_por_fold.csv')
